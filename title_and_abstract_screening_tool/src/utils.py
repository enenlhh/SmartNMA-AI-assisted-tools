...
def load_config(config_path):
    ...
    try:
        ...
        # Simple cleanup: only remove BOM
        if content.startswith('\ufeff'):
            content = content[1:]
        
        # Parse JSON directly without excessive processing
        ...
        # Show problematic line from original file
        try:
            with open(config_path, 'r', encoding='utf-8') as f:
                lines = f.readlines()
            if e.lineno <= len(lines):
                ...
                # Show hexadecimal representation of characters
